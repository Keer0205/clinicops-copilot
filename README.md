...
ClinicOps Copilot is a **Python and Streamlit** application that lets an aesthetic clinic upload internal PDFs such as SOPs, consent forms, aftercare guides, and pricing documents, then ask questions and receive **citation-backed answers**. The assistant answers **only from the uploaded documents** and returns **page citations** so staff can verify information quickly.

## Business Problem
Clinic staff often need to search across multiple SOPs, consent forms, aftercare guides, and pricing PDFs to answer operational questions. This can be slow, inconsistent, and error-prone. This project was built to reduce manual searching and provide faster, grounded answers based only on approved clinic documents.

## Solution Overview
The application ingests uploaded clinic PDFs, extracts text, indexes the content into a searchable vector store, and retrieves relevant passages to generate grounded answers with page citations. If the answer is not present in the source documents, the assistant refuses rather than guessing.
...
4. Review the answer and page citations  
...
- Evaluation mode for regression testing
